...
def render_timeline_chart(filtered_df: pd.DataFrame) -> None:
    ...
    fig_timeline = px.line(df_timeline, x='Datum', y='Gesamtzeit_h', markers=True, 
                           ...
                           line_shape="spline", color_discrete_sequence=['#829eb9'])
    fig_timeline.update_layout(yaxis_range=[0, max(filtered_df['Gesamtzeit_h'].max() * 1.2, 1)], 
...
def render_apps_stacked_bar(app_df: pd.DataFrame) -> None:
    ...
    fig_apps = px.bar(cat_df_timeline, x='Datum', y='App_Zeit_h', color='Kategorie',
                      labels={"App_Zeit_h": "Zeit (Stunden)", "Kategorie": "Kategorie", "Datum": "Datum"},
                      title="Tagesverlauf der Kategorien-Nutzung",
                      color_discrete_sequence=px.colors.qualitative.Pastel)
    fig_apps.update_layout(template='simple_white', barmode='stack', font=dict(family="Fredoka, sans-serif"))
...
def render_small_charts(filtered_df: pd.DataFrame, app_df: pd.DataFrame) -> None:
    ...
    with col_chart1:
        ...
        week_order = ['Montag', 'Dienstag', 'Mittwoch', 'Donnerstag', 'Freitag', 'Samstag', 'Sonntag']
        existing_days = [d for d in week_order if d in weekday_grouped['Wochentag'].values]
        
        if not weekday_grouped.empty:
            fig_weekdays = px.bar(weekday_grouped, x='Wochentag', y='Gesamtzeit_h', 
                                  category_orders={'Wochentag': existing_days},
                                  labels={"Gesamtzeit_h": "Ø Zeit (Stunden)", "Wochentag": "Tag"},
                                  color_discrete_sequence=['#a3b8cc'])
            fig_weekdays.update_layout(template='simple_white', font=dict(family="Fredoka, sans-serif"))
    ...
    with col_chart2:
        ...
        if not app_df.empty:
            ...
            fig_pie = px.pie(cat_sorted, values='App_Zeit_h', names='Kategorie', hole=0.5,
                             color_discrete_sequence=px.colors.qualitative.Pastel)
            fig_pie.update_traces(textposition='inside', textinfo='percent+label')
            fig_pie.update_layout(template='simple_white', showlegend=False, font=dict(family="Fredoka, sans-serif"))
