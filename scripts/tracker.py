...
def main() -> None:
    ...
    while True:
        try:
            num_days = int(input("Wie viele Tage möchtest du dieses Mal eintragen? (z.B. 1 oder 7): ").strip())
            if num_days > 0:
                break
        except ValueError:
    ...
    for d in range(num_days):
        ...
        total_time = input(f"Gesamte Bildschirmzeit am {wochentag} (z.B. 4h 30m): ").strip()
    ...
    for row in records:
        ...
        ''', (
            ...
            row.get('App4_Name', ''), row.get('App4_Zeit', ''),
            row.get('App5_Name', ''), row.get('App5_Zeit', '')
        ))
